...
//
// Please share any improvements and/or bug fixes.
//
// v0.17 .. 28th March 2011
...
// __interrupt void timer1CompAInterrupt(void)
...
// __interrupt void usartRxInterrupt(void)
ISR(USART0_RX_vect){
...
{	// setup the RS232 port
	...
	UBRR0 = (uint16_t)((FOSC / (16.0 * UART_BAUDRATE)) - 0.5);
...
//	useFailSafeValues();								// use the fail-safe values while scanning for a transmitter
	...
	while (chan <= END_CHAN)
	{
        ...
        _delay_ms(30);	// wait 30ms .. give the PLL time to lock - takes around 25ms for large frequency change
            ...
            // __delay_cycles((uint32_t)(FOSC * 0.01));	// wait 10ms .. give the PLL time to lock - takes around 6ms for 10kHz change in frequency
...
//	WDTCSR |= (1<<WDIF) | (1<<WDIE);
    ...
    // __delay_cycles((uint32_t)(FOSC * 0.05));		// wait 50ms
    _delay_ms(50);
	...
	while (true)
	{
		...
		{
			LED_PORT |= 1 << LED_BIT;						// led ON
            // __delay_cycles((uint32_t)(FOSC * 0.07));		// wait 70ms
            _delay_ms(70);
			LED_PORT &= ~(1 << LED_BIT);					// led OFF
            // __delay_cycles((uint32_t)(FOSC * 0.15));		// wait 150ms
            _delay_ms(150);
		}
